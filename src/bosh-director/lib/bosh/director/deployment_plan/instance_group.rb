...
module Bosh::Director
  module DeploymentPlan
    class InstanceGroup
      ...
      # needs to be modified. The only valid keys in a job spec are the 'name'
      # and 'templates' keys. All other keys are discarded.
      def self.convert_from_legacy_spec(job_spec)
        job_spec.select { |k, _| %w[name templates].include?(k) }
      end
